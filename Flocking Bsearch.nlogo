...
globals [ avg-dx-history avg-dy-history ]
...
to go
  ...
  ;;   ask turtles [ fd 1 ]
  set avg-dx-history fput (mean [dx] of turtles) avg-dx-history
  if (length avg-dx-history > 100)
  [ set avg-dx-history sublist avg-dx-history 0 100 ]
  set avg-dy-history fput (mean [dy] of turtles) avg-dy-history
  if (length avg-dy-history > 100)
  [ set avg-dy-history sublist avg-dy-history 0 100 ]
  tick
  if plotting?
  [ 
    plot alignment-stdev
    ; plot mean [ dx ] of turtles
  ]
end

to-report alignment-instability-over-time
  report (standard-deviation avg-dx-history) + (standard-deviation avg-dy-history)
end

to-report alignment-stdev
  report standard-deviation [ dx ] of turtles + standard-deviation [ dy ] of turtles 
end
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="false">
    ...
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>alignment-instability-over-time</metric>
