...
to-report corr-coeff [ list1 list2 ]
  ...
  let numerator sum (map [ (?1 - mean1) * (?2 - mean2) ] list1 list2)
...
FORWARD NOTE
----
This is the BehaviorSearch benchmarking version of the classic NetLogo Fireflies model.  Modifications: 
...
@#$#@#$#@
<experiments>
  <experiment name="factorial_too_big" repetitions="1" runMetricsEveryStep="false">
    <setup>let x random 100</setup>
    <timeLimit steps="1"/>
    <metric>calibrate</metric>
    <enumeratedValueSet variable="grass?">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    <steppedValueSet variable="grass-regrowth-time" first="0" step="20" last="100"/>
    <steppedValueSet variable="initial-number-sheep" first="50" step="50" last="250"/>
    <steppedValueSet variable="initial-number-wolves" first="50" step="50" last="250"/>
    <steppedValueSet variable="sheep-gain-from-food" first="10" step="10" last="50"/>
    <steppedValueSet variable="wolf-gain-from-food" first="20" step="20" last="100"/>
    <steppedValueSet variable="sheep-reproduce" first="1" step="1" last="10"/>
    <steppedValueSet variable="wolf-reproduce" first="1" step="1" last="10"/>
    <steppedValueSet variable="ticks-per-year" first="2" step="2" last="20"/>
